...
class TProp extends DbTable
{
    ...
    public function __construct()
    {
        ...
        $this->prop->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->prop->Param, "CustomMsg");
        ...
        $this->kdpropold->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->kdpropold->Param, "CustomMsg");
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->kdprop->setDbValue($conn->lastInsertId());
    ...
    {
        $keys = [];
        $val = $current ? $this->kdprop->CurrentValue : $this->kdprop->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->kdprop->CurrentValue = $keys[0];
            } else {
                $this->kdprop->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("tproplist");
    }
    ...
    {
        global $Language;
        if ($pageName == "tpropview") {
            return $Language->phrase("View");
        } elseif ($pageName == "tpropedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "tpropadd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("tpropview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("tpropview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "tpropadd?" . $this->getUrlParm($parm);
        } else {
            $url = "tpropadd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("tpropedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("tpropadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("tpropdelete", $this->getUrlParm());
    }
    ...
    public function keyUrl($url, $parm = "")
    {
        if ($this->kdprop->CurrentValue !== null) {
            $url .= "/" . rawurlencode($this->kdprop->CurrentValue);
        } else {
...
SORTHTML;
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("kdprop") ?? Route("kdprop")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function loadListRowValues(&$rs)
    {
        ...
        $this->prop->setDbValue($row['prop']);
        $this->kdpropold->setDbValue($row['kdpropold']);
    ...
    public function renderListRow()
    {
        ...
        $this->kdpropold->ViewValue = FormatNumber($this->kdpropold->ViewValue, 0, -2, -2, -2);
