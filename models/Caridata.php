...
/**
 * Table class for caridata
 */
class Caridata extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'caridata';
        $this->TableName = 'caridata';
        $this->TableType = 'VIEW';

        // Update Table
        $this->UpdateTable = "`caridata`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // judul
        $this->judul = new DbField('caridata', 'caridata', 'x_judul', 'judul', '`judul`', '`judul`', 200, 255, -1, false, '`judul`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // isi
        $this->isi = new DbField('caridata', 'caridata', 'x_isi', 'isi', '`isi`', '`isi`', 201, -1, -1, false, '`isi`', false, false, false, 'FORMATTED TEXT', 'TEXTAREA');
        $this->isi->Nullable = false; // NOT NULL field
        $this->isi->Required = true; // Required field
        $this->isi->Sortable = true; // Allow sort
        $this->isi->MemoMaxLength = 150;
        $this->isi->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->isi->Param, "CustomMsg");
        $this->Fields['isi'] = &$this->isi;

        // tanggal
        $this->tanggal = new DbField('caridata', 'caridata', 'x_tanggal', 'tanggal', '`tanggal`', '`tanggal`', 200, 10, -1, false, '`tanggal`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->tanggal->Sortable = true; // Allow sort
        $this->tanggal->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->tanggal->Param, "CustomMsg");
        $this->Fields['tanggal'] = &$this->tanggal;

        // jenis
        $this->jenis = new DbField('caridata', 'caridata', 'x_jenis', 'jenis', '`jenis`', '`jenis`', 200, 9, -1, false, '`jenis`', false, false, false, 'FORMATTED TEXT', 'CHECKBOX');
        $this->jenis->Nullable = false; // NOT NULL field
        $this->jenis->Required = true; // Required field
        $this->jenis->Sortable = true; // Allow sort
        $this->jenis->Lookup = new Lookup('jenis', 'caridata', false, '', ["","","",""], [], [], [], [], [], [], '', '');
        ...
        // gambar
        $this->gambar = new DbField('caridata', 'caridata', 'x_gambar', 'gambar', '`gambar`', '`gambar`', 201, 16777215, -1, false, '`gambar`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->gambar->Sortable = true; // Allow sort
        $this->gambar->ImageResize = true;
        $this->gambar->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->gambar->Param, "CustomMsg");
        $this->Fields['gambar'] = &$this->gambar;

        // id
        $this->id = new DbField('caridata', 'caridata', 'x_id', 'id', '`id`', '`id`', 3, 11, -1, false, '`id`', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`caridata`";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("caridatalist");
    }
    ...
    {
        global $Language;
        if ($pageName == "caridataview") {
            return $Language->phrase("View");
        } elseif ($pageName == "caridataedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "caridataadd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("caridataview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("caridataview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "caridataadd?" . $this->getUrlParm($parm);
        } else {
            $url = "caridataadd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("caridataedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("caridataadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("caridatadelete", $this->getUrlParm());
    }
...
SORTHTML;
    ...
    public function renderListRow()
    {
        ...
        // tanggal
        $this->tanggal->CellCssStyle = "white-space: nowrap;";
        ...
        if ($this->isi->ViewValue != null) {
            $this->isi->ViewValue = str_replace(["\r\n", "\n", "\r"], "<br>", $this->isi->ViewValue);
        }
        ...
        $this->gambar->ViewCustomAttributes = "width='100%'";
    ...
    public function renderEditRow()
    {
        ...
        if (!$this->tanggal->Raw) {
            $this->tanggal->CurrentValue = HtmlDecode($this->tanggal->CurrentValue);
        }
    ...
    public function rowRendered()
    {
    	...
    	$path_gambar = "images/news/";
    	$link = "<a href='berita?baca=".$this->id->CurrentValue."'>";
    	if($this->jenis->CurrentValue == "pelatihan"){
    		$path_gambar = "files/";
    		$link = "<a href='detail-pelatihan/view/".$this->id->CurrentValue."'>";
    	}
    	$this->tanggal->ViewValue = tanggal_indo($this->tanggal->ViewValue);
    	$gambar = explode(Config("MULTIPLE_UPLOAD_SEPARATOR") ,$this->gambar->CurrentValue);
    	$this->gambar->ViewValue = "<img src='".$path_gambar.$gambar[0]."' width='100%' height='145px'>";
    	$this->id->ViewValue = $link.'<div class="row my-3"><div class="col-2">'.$this->gambar->ViewValue.'</div><div class="col-10">
    	<h4 class="text-bold">'.$this->judul->ViewValue.'</h4>
    	'.RemoveHTML($this->isi->ViewValue).'<br><i>'.$this->tanggal->ViewValue.'</i> <span class="badge badge-light">'.$this->jenis->ViewValue.'</span><br><br></div>
    	</div></a>';
    }
