 ...
 * Page class
 */
class WPelatihanEdit extends WPelatihan
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "WPelatihanEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("pelatihan_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->pelatihan_id->setQueryStringValue($keyValue);
                $this->pelatihan_id->setOldValue($this->pelatihan_id->QueryStringValue);
            } elseif (Post("pelatihan_id") !== null) {
                $this->pelatihan_id->setFormValue(Post("pelatihan_id"));
                $this->pelatihan_id->setOldValue($this->pelatihan_id->FormValue);
            } else {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->Last_Updated->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->Last_Updated->setFormValue($val);
            }
            $this->Last_Updated->CurrentValue = UnFormatDateTime($this->Last_Updated->CurrentValue, 0);
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // jenis_pelatihan
            $this->jenis_pelatihan->setDbValueDef($rsnew, $this->jenis_pelatihan->CurrentValue, "", $this->jenis_pelatihan->ReadOnly);

            // judul_pelatihan
            $this->judul_pelatihan->setDbValueDef($rsnew, $this->judul_pelatihan->CurrentValue, "", $this->judul_pelatihan->ReadOnly);

            // jumlah_hari
            $this->jumlah_hari->setDbValueDef($rsnew, $this->jumlah_hari->CurrentValue, "", $this->jumlah_hari->ReadOnly);

            // tempat
            $this->tempat->setDbValueDef($rsnew, $this->tempat->CurrentValue, "", $this->tempat->ReadOnly);

            // jumlah_peserta
            $this->jumlah_peserta->setDbValueDef($rsnew, $this->jumlah_peserta->CurrentValue, 0, $this->jumlah_peserta->ReadOnly);
            ...
            // tawal
            $this->tawal->setDbValueDef($rsnew, UnFormatDateTime($this->tawal->CurrentValue, 7), null, $this->tawal->ReadOnly);

            // takhir
            $this->takhir->setDbValueDef($rsnew, UnFormatDateTime($this->takhir->CurrentValue, 7), null, $this->takhir->ReadOnly);

            // tanggal_pelaksanaan
            $this->tanggal_pelaksanaan->setDbValueDef($rsnew, $this->tanggal_pelaksanaan->CurrentValue, "", $this->tanggal_pelaksanaan->ReadOnly);
            ...
            // sub_kategori
            $this->sub_kategori->setDbValueDef($rsnew, $this->sub_kategori->CurrentValue, "", $this->sub_kategori->ReadOnly);

            // topik_bahasan
            $this->topik_bahasan->setDbValueDef($rsnew, $this->topik_bahasan->CurrentValue, "", $this->topik_bahasan->ReadOnly);
            ...
            // Last_Updated
            $this->Last_Updated->CurrentValue = CurrentDateTime();
            $this->Last_Updated->setDbValueDef($rsnew, $this->Last_Updated->CurrentValue, CurrentDate());
            ...
            $this->Activated->setDbValueDef($rsnew, $tmpBool, null, $this->Activated->ReadOnly);
            if ($this->gambar->Visible && !$this->gambar->Upload->KeepFile) {
                ...
                if (!EmptyValue($this->gambar->Upload->FileName)) {
                    ...
                    $this->gambar->setDbValueDef($rsnew, $this->gambar->Upload->FileName, "", $this->gambar->ReadOnly);
                }
            }
