 ...
 * Page class
 */
class WOrdersAdd extends WOrders
{
    use MessagesTrait;
    ...
    public function run()
    {
        ...
        $this->bukti_pembayaran->Visible = false;
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("worderslist"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    $returnUrl = "konfimasipembayaranedit?order_id=". urlencode($this->order_id->CurrentValue);
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->tgl_tranfer->CurrentValue = null;
        $this->tgl_tranfer->OldValue = $this->tgl_tranfer->CurrentValue;
        $this->bukti_pembayaran->Upload->DbValue = null;
        $this->bukti_pembayaran->OldValue = $this->bukti_pembayaran->Upload->DbValue;
        $this->bukti_pembayaran->CurrentValue = null; // Clear file related field
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->pelatihan_id->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->pelatihan_id->Visible = false; // Disable update for API request
            } else {
                $this->pelatihan_id->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->pelatihan_id->CurrentValue = $this->pelatihan_id->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row['tgl_tranfer'] = $this->tgl_tranfer->CurrentValue;
        $row['bukti_pembayaran'] = $this->bukti_pembayaran->Upload->DbValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($curVal != "") {
                ...
                $this->pelatihan_id->ViewValue = $this->pelatihan_id->Lookup !== null && is_array($this->pelatihan_id->Lookup->Options) ? $curVal : null;
            }
            if ($this->pelatihan_id->ViewValue !== null) { // Load from cache
                $this->pelatihan_id->EditValue = array_values($this->pelatihan_id->Lookup->Options);
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "`pelatihan_id`" . SearchString("=", $this->pelatihan_id->CurrentValue, DATATYPE_NUMBER, "");
                }
                $lookupFilter = function() {
                    return "`Activated` = 'Y'";
                };
    ...
    protected function validateForm()
    {
        ...
        if ($this->pelatihan_id->Required) {
            if (!$this->pelatihan_id->IsDetailKey && EmptyValue($this->pelatihan_id->FormValue)) {
                $this->pelatihan_id->addErrorMessage(str_replace("%s", $this->pelatihan_id->caption(), $this->pelatihan_id->RequiredErrorMessage));
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        $this->_username->setDbValueDef($rsnew, $this->_username->CurrentValue, "");

        // nama_perusahaan
        $this->nama_perusahaan->setDbValueDef($rsnew, $this->nama_perusahaan->CurrentValue, "", false);

        // pelatihan_id
        $this->pelatihan_id->setDbValueDef($rsnew, $this->pelatihan_id->CurrentValue, 0, false);

        // Biaya
        $this->Biaya->setDbValueDef($rsnew, $this->Biaya->CurrentValue, null, false);

        // tgl_tranfer
        $this->tgl_tranfer->setDbValueDef($rsnew, UnFormatDateTime($this->tgl_tranfer->CurrentValue, 0), null, false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("worderslist"), "", $this->TableVar, true);
