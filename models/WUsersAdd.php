 ...
 * Page class
 */
class WUsersAdd extends WUsers
{
    use MessagesTrait;
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("wuserslist"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "wuserslist") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "wusersview") {
    ...
    protected function validateForm()
    {
        ...
        if ($this->pass->Required) {
            if (!$this->pass->IsDetailKey && EmptyValue($this->pass->FormValue)) {
                $this->pass->addErrorMessage(str_replace("%s", $this->pass->caption(), $this->pass->RequiredErrorMessage));
            }
        }
        ...
        if ($this->user_created_by->Required) {
            if (!$this->user_created_by->IsDetailKey && EmptyValue($this->user_created_by->FormValue)) {
                $this->user_created_by->addErrorMessage(str_replace("%s", $this->user_created_by->caption(), $this->user_created_by->RequiredErrorMessage));
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // userlevel
        if ($Security->canAdmin()) { // System admin
            $this->_userlevel->setDbValueDef($rsnew, $this->_userlevel->CurrentValue, 0, strval($this->_userlevel->CurrentValue) == "");
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("wuserslist"), "", $this->TableVar, true);
