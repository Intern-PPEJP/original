 ...
 * Page class
 */
class WPagesList extends WPages
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "WPagesList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fw_pageslist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "wpagesadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "wpagesdelete";
        $this->MultiUpdateUrl = "wpagesupdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fw_pageslistsrch";
    ...
    public function run()
    {
        ...
        $this->page_content->Visible = false;
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->page_id->AdvancedSearch->toJson(), ","); // Field page_id
        $filterList = Concat($filterList, $this->page_name->AdvancedSearch->toJson(), ","); // Field page_name
        $filterList = Concat($filterList, $this->page_content->AdvancedSearch->toJson(), ","); // Field page_content
        $filterList = Concat($filterList, $this->custom_url->AdvancedSearch->toJson(), ","); // Field custom_url
        $filterList = Concat($filterList, $this->external_link->AdvancedSearch->toJson(), ","); // Field external_link
        $filterList = Concat($filterList, $this->updated_date->AdvancedSearch->toJson(), ","); // Field updated_date
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fw_pageslistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field custom_url
        $this->custom_url->AdvancedSearch->SearchValue = @$filter["x_custom_url"];
        $this->custom_url->AdvancedSearch->SearchOperator = @$filter["z_custom_url"];
        $this->custom_url->AdvancedSearch->SearchCondition = @$filter["v_custom_url"];
        $this->custom_url->AdvancedSearch->SearchValue2 = @$filter["y_custom_url"];
        $this->custom_url->AdvancedSearch->SearchOperator2 = @$filter["w_custom_url"];
        $this->custom_url->AdvancedSearch->save();

        // Field external_link
        $this->external_link->AdvancedSearch->SearchValue = @$filter["x_external_link"];
        $this->external_link->AdvancedSearch->SearchOperator = @$filter["z_external_link"];
        $this->external_link->AdvancedSearch->SearchCondition = @$filter["v_external_link"];
        $this->external_link->AdvancedSearch->SearchValue2 = @$filter["y_external_link"];
        $this->external_link->AdvancedSearch->SearchOperator2 = @$filter["w_external_link"];
        $this->external_link->AdvancedSearch->save();
    ...
    {
        $where = "";
        $this->buildBasicSearchSql($where, $this->page_name, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->page_content, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->custom_url, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->external_link, $arKeywords, $type);
        return $where;
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->custom_url); // custom_url
            $this->updateSort($this->external_link); // external_link
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->page_content->setSort("");
                $this->custom_url->setSort("");
                $this->external_link->setSort("");
    ...
    public function renderListOptions()
    {
        ...
        $opt->Body = "<div class=\"custom-control custom-checkbox d-inline-block\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"custom-control-input ew-multi-select\" value=\"" . HtmlEncode($this->custom_url->CurrentValue) . "\" onclick=\"ew.clickMultiCheckbox(event);\"><label class=\"custom-control-label\" for=\"key_m_" . $this->RowCount . "\"></label></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fw_pageslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fw_pageslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fw_pageslist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fw_pageslistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
