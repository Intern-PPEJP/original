 ...
 * Page class
 */
class WTestimoniEdit extends WTestimoni
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "WTestimoniEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("testimoni_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->testimoni_id->setQueryStringValue($keyValue);
                $this->testimoni_id->setOldValue($this->testimoni_id->QueryStringValue);
            } elseif (Post("testimoni_id") !== null) {
                $this->testimoni_id->setFormValue(Post("testimoni_id"));
                $this->testimoni_id->setOldValue($this->testimoni_id->FormValue);
            } else {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->testimoni_id->IsDetailKey) {
            $this->testimoni_id->setFormValue($val);
        }
    ...
    {
        global $CurrentForm;
        $this->testimoni_id->CurrentValue = $this->testimoni_id->FormValue;
    ...
    protected function validateForm()
    {
        ...
        if ($this->testimoni_id->Required) {
            if (!$this->testimoni_id->IsDetailKey && EmptyValue($this->testimoni_id->FormValue)) {
                $this->testimoni_id->addErrorMessage(str_replace("%s", $this->testimoni_id->caption(), $this->testimoni_id->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // testimoni
            $this->testimoni->setDbValueDef($rsnew, $this->testimoni->CurrentValue, "", $this->testimoni->ReadOnly);

            // link_testimoni
            $this->link_testimoni->setDbValueDef($rsnew, $this->link_testimoni->CurrentValue, null, $this->link_testimoni->ReadOnly);
            ...
            $this->show->setDbValueDef($rsnew, $tmpBool, "N", $this->show->ReadOnly);
