 ...
 * Page class
 */
class WUsersList extends WUsers
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'w_users';

    // Page object name
    public $PageObjName = "WUsersList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fw_userslist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "wusersadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "wusersdelete";
        $this->MultiUpdateUrl = "wusersupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'w_users');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fw_userslistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("w_users"));
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        $this->updated_at->Visible = false;
        $this->user_updated_by->Visible = false;
        $this->nama_peserta->Visible = false;
        $this->perusahaan->Visible = false;
        $this->jabatan->Visible = false;
        $this->provinsi->Visible = false;
        ...
        // Set up lookup cache
        $this->setupLookupOptions($this->pass);
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->user_created_by->AdvancedSearch->toJson(), ","); // Field user_created_by
        $filterList = Concat($filterList, $this->updated_at->AdvancedSearch->toJson(), ","); // Field updated_at
        $filterList = Concat($filterList, $this->user_updated_by->AdvancedSearch->toJson(), ","); // Field user_updated_by
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fw_userslistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        $this->user_created_by->AdvancedSearch->SearchValue = @$filter["x_user_created_by"];
        $this->user_created_by->AdvancedSearch->SearchOperator = @$filter["z_user_created_by"];
        $this->user_created_by->AdvancedSearch->SearchCondition = @$filter["v_user_created_by"];
        $this->user_created_by->AdvancedSearch->SearchValue2 = @$filter["y_user_created_by"];
        $this->user_created_by->AdvancedSearch->SearchOperator2 = @$filter["w_user_created_by"];
        $this->user_created_by->AdvancedSearch->save();
        ...
        $this->user_updated_by->AdvancedSearch->SearchValue = @$filter["x_user_updated_by"];
        $this->user_updated_by->AdvancedSearch->SearchOperator = @$filter["z_user_updated_by"];
        $this->user_updated_by->AdvancedSearch->SearchCondition = @$filter["v_user_updated_by"];
        $this->user_updated_by->AdvancedSearch->SearchValue2 = @$filter["y_user_updated_by"];
        $this->user_updated_by->AdvancedSearch->SearchOperator2 = @$filter["w_user_updated_by"];
        $this->user_updated_by->AdvancedSearch->save();
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->user_created_by, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->user_updated_by, $arKeywords, $type);
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->user_created_by->setSort("");
                $this->updated_at->setSort("");
                $this->user_updated_by->setSort("");
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fw_userslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fw_userslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fw_userslist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fw_userslistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
