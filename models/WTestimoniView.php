 ...
 * Page class
 */
class WTestimoniView extends WTestimoni
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "WTestimoniView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("testimoni_id") ?? Route("testimoni_id")) !== null) {
            $this->RecKey["testimoni_id"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("testimoni_id") ?? Route("testimoni_id")) !== null) {
                ...
                $this->RecKey["testimoni_id"] = $this->testimoni_id->QueryStringValue;
            } elseif (Post("testimoni_id") !== null) {
                $this->testimoni_id->setFormValue(Post("testimoni_id"));
                $this->RecKey["testimoni_id"] = $this->testimoni_id->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->testimoni_id->setQueryStringValue($keyValue);
                $this->RecKey["testimoni_id"] = $this->testimoni_id->QueryStringValue;
            } else {
                $returnUrl = "wtestimonilist"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "wtestimonilist"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "wtestimonilist"; // Not page request, return to list
        }
        if ($returnUrl != "") {
