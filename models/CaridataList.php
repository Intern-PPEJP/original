 ...
 * Page class
 */
class CaridataList extends Caridata
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'caridata';

    // Page object name
    public $PageObjName = "CaridataList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fcaridatalist";
    ...
    public function __construct()
    {
        ...
        // Table object (caridata)
        if (!isset($GLOBALS["caridata"]) || get_class($GLOBALS["caridata"]) == PROJECT_NAMESPACE . "caridata") {
            $GLOBALS["caridata"] = &$this;
        }
        ...
        $this->AddUrl = "caridataadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "caridatadelete";
        $this->MultiUpdateUrl = "caridataupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'caridata');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fcaridatalistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("caridata"));
                $doc->Text = @$content;
    ...
    public $PageSizes = "5,10,20"; // Page sizes (comma separated)
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fcaridatalistsrch", $filters);
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->jenis, $arKeywords, $type);
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fcaridatalistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fcaridatalistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fcaridatalist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // isi
            $this->isi->ViewValue = TruncateMemo($this->isi->CurrentValue, 150, $this->isi->TruncateMemoRemoveHtml);
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fcaridatalistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    public function pageDataRendering(&$header)
    {
        // Example:
        $header = '<style>
    .main-header,.content-header,.main-footer,.mb-2 .text-dark, .breadcrumb,#fcari { display:none !important; }
    .ew-ext-search-form,.ew-search-panel,ew-quick-search  { width:100% !important; }
    #psearch { width: 60% !important; }
    .ew-card { width: 100%;border:none; }
    .gambar { float:left; margin-right: 10px; }
    .ew-custom-template ol,.ew-custom-template ul { list-style-position: inside; }
    .ew-custom-template a { color:#212529; }
    .ew-custom-template a:hover { text-decoration:none !important; color:#0056b3; }
