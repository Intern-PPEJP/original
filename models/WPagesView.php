 ...
 * Page class
 */
class WPagesView extends WPages
{
    use MessagesTrait;
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("custom_url") ?? Route("custom_url")) !== null) {
            $this->RecKey["custom_url"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("custom_url") ?? Route("custom_url")) !== null) {
                ...
                $this->RecKey["custom_url"] = $this->custom_url->QueryStringValue;
            } elseif (Post("custom_url") !== null) {
                $this->custom_url->setFormValue(Post("custom_url"));
                $this->RecKey["custom_url"] = $this->custom_url->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->custom_url->setQueryStringValue($keyValue);
                $this->RecKey["custom_url"] = $this->custom_url->QueryStringValue;
            } else {
                $returnUrl = "wpageslist"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "wpageslist"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "wpageslist"; // Not page request, return to list
        }
        if ($returnUrl != "") {
