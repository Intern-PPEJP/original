...
<div class="container-fluid " style="background-color: #3A8F53; padding:20px 0px;">
    <div class="container" style="xmargin-top:110px">
        <div class="row" style="xbackground-color: #031A31;">
            <div class="col-md-12">
                <h1 class="m-0" style="color: white;font-weight:bold;">PELATIHAN MUTU</h1>
            </div>
        </div>
    </div>
</div>
...
<section class="content-section mt-5">
	<div class="container">
		...
		<?php
			$rs = ExecuteQuery("SELECT `pelatihan_id`, `judul_pelatihan`, `jumlah_hari`,`sisa`, `tempat`, `jumlah_peserta`, `harga`, `tanggal_pelaksanaan`, `gambar`, `Last_Updated`, `Created_Date` FROM `w_pelatihan` WHERE `Activated` = 'Y' AND `tawal` >= CURRENT_DATE() AND `jenis_pelatihan` LIKE 'mutu'");
			$jumlahpelatihan = 0;
			while ($row = $rs->fetch()) {
