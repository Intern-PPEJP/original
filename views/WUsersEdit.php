...
loadjs.ready("head", function () {
    ...
    fw_usersedit = currentForm = new ew.Form("fw_usersedit", "edit");
    ...
    // Validate form
    fw_usersedit.validate = function () {
    ...
    // Form_CustomValidate
    fw_usersedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fw_usersedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fw_usersedit.lists.pass = <?= $Page->pass->toClientList($Page) ?>;
    fw_usersedit.lists._userlevel = <?= $Page->_userlevel->toClientList($Page) ?>;
    fw_usersedit.lists.aktif = <?= $Page->aktif->toClientList($Page) ?>;
    loadjs.done("fw_usersedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fw_usersedit" id="fw_usersedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->user_id->Visible) { // user_id ?>
    <div id="r_user_id" class="form-group row">
        <label id="elh_w_users_user_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->user_id->caption() ?><?= $Page->user_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->user_id->cellAttributes() ?>>
<span id="el_w_users_user_id">
...
<input type="hidden" data-table="w_users" data-field="x_user_id" data-hidden="1" name="x_user_id" id="x_user_id" value="<?= HtmlEncode($Page->user_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->aktif->isInvalidClass() ?>" data-table="w_users" data-field="x_aktif" name="x_aktif[]" id="x_aktif_723494" value="1"<?= ConvertToBool($Page->aktif->CurrentValue) ? " checked" : "" ?><?= $Page->aktif->editAttributes() ?> aria-describedby="x_aktif_help">
    <label class="custom-control-label" for="x_aktif_723494"></label>
</div>
