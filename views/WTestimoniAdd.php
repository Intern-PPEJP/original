...
loadjs.ready("head", function () {
    ...
    fw_testimoniadd = currentForm = new ew.Form("fw_testimoniadd", "add");
    ...
    // Validate form
    fw_testimoniadd.validate = function () {
    ...
    // Form_CustomValidate
    fw_testimoniadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fw_testimoniadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fw_testimoniadd.lists.show = <?= $Page->show->toClientList($Page) ?>;
    loadjs.done("fw_testimoniadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fw_testimoniadd" id="fw_testimoniadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->show->isInvalidClass() ?>" data-table="w_testimoni" data-field="x_show" name="x_show[]" id="x_show_830653" value="1"<?= ConvertToBool($Page->show->CurrentValue) ? " checked" : "" ?><?= $Page->show->editAttributes() ?> aria-describedby="x_show_help">
    <label class="custom-control-label" for="x_show_830653"></label>
</div>
