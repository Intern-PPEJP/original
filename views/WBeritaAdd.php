...
loadjs.ready("head", function () {
    ...
    fw_beritaadd = currentForm = new ew.Form("fw_beritaadd", "add");
    ...
    fw_beritaadd.addFields([
        ...
        ["user_created", [fields.user_created.visible && fields.user_created.required ? ew.Validators.required(fields.user_created.caption) : null], fields.user_created.isInvalid],
    ...
    // Validate form
    fw_beritaadd.validate = function () {
    ...
    // Form_CustomValidate
    fw_beritaadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fw_beritaadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fw_beritaadd.lists.kategori_id = <?= $Page->kategori_id->toClientList($Page) ?>;
    fw_beritaadd.lists.publish = <?= $Page->publish->toClientList($Page) ?>;
    loadjs.done("fw_beritaadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fw_beritaadd" id="fw_beritaadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<script>
loadjs.ready(["fw_beritaadd", "editor"], function() {
	ew.createEditor("fw_beritaadd", "x_isi", 35, 8, <?= $Page->isi->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
...
<script>
loadjs.ready(["fw_beritaadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fw_beritaadd", "x_tanggal_publikasi", {"ignoreReadonly":true,"useCurrent":false,"format":14});
});
</script>
<?php } ?>
</span>
...
<div class="custom-control custom-checkbox d-inline-block">
    <input type="checkbox" class="custom-control-input<?= $Page->publish->isInvalidClass() ?>" data-table="w_berita" data-field="x_publish" name="x_publish[]" id="x_publish_254114" value="1"<?= ConvertToBool($Page->publish->CurrentValue) ? " checked" : "" ?><?= $Page->publish->editAttributes() ?> aria-describedby="x_publish_help">
    <label class="custom-control-label" for="x_publish_254114"></label>
</div>
